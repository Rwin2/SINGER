#!/usr/bin/env python3
"""
Render annotated videos with plain EMA (best overall config from fixed_pipeline):
  - OWL-ViT v1 with rich prompts
  - EMA alpha=0.5, coast=30, NO spatial gating

Usage:
    cd /data/erwinpi/SINGER
    CUDA_VISIBLE_DEVICES=0 PYTHONPATH=scripts:$PYTHONPATH \
    conda run --no-capture-output -n FiGS python -u scripts/perception_debug/render_ema_only.py
"""
...
def render_case(case, proc, mdl, dev, out_dir):
    filt = EMAFilter(alpha=0.5, conf_gate=0.02, max_coast=30)
    ...
    while True:
        ...
        if est is not None:
            mode = "meas" if filt.steps_coasting == 0 else f"coast({filt.steps_coasting})"
            txt2 = f"EMA: {mode}"
        else:
            txt2 = "EMA: not init"
        parts = []
